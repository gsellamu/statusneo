...
$exists = docker exec $PgContainer psql -U $PgUser -d postgres -tAc "SELECT 1 FROM pg_database WHERE datname='$Database'" 2>$null
if ($exists -match "1") {
  Ok "database '$Database' already exists"
} else {
  docker exec $PgContainer psql -U $PgUser -d postgres -c "CREATE DATABASE $Database;" | Out-Null
