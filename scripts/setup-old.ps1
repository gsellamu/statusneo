...
# Wrap the command in parentheses to safely redirect errors to null
$exists = (docker exec $PgContainer psql -U jeethhypno_user -p 5432 -d jeethhypno -tAc "SELECT 1 FROM pg_database WHERE datname='$Database'") 2>$null

if ($exists -eq "1") { 
    Ok "database '$Database' already exists" 
} else { 
    docker exec $PgContainer psql -U jeethhypno_user -p 5432 -d jeethhypno -c "CREATE DATABASE $Database;" | Out-Null 
    
    if ($LASTEXITCODE -eq 0) { 
        Ok "created database '$Database'" 
    } else { 
        Warn "could not create DB - check the postgres password; edit appsettings.Real.json if not 'postgres'" 
    } 
}
