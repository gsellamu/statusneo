...
.\scripts\demo-reset.ps1          # clean rooms + reseed Neo4j (your existing containers)
.\scripts\run.ps1 -Port 5005      # twin service on host -> your live Postgres/Neo4j/Ollama
# then open http://localhost:5005 (and a 2nd tab for live-sync)
```
> No `docker compose` needed — your infra is already running; the service connects to it.
...
## 12. THE MVP MOMENT: Room Moducules — design once, reuse everywhere

This is Modutecture's reason to exist, running live. Your own architecture calls it
"BaseModucules (Get/Create)" — the reusable template catalog. Here it is, working.

**The concept (say it first):** a Moducule is a reusable, rule-bearing lego block at THREE
levels — *content* (headwall, bed, sink), *room* (a whole "Standard ICU Room" template), and
*layout*. A room is itself a Moducule. The relationship model: a **tree** for containment
(Building->Floor->Room->component), a **composition DAG** for reusable types (shared types have
many parents — the blast radius), and **validated directed edges** for relationships (med-gas).
"Trees where you own, DAGs where you reuse, edges where you connect."

**The demo (on /hierarchy.html, after a reset so rooms are EMPTY):**
1. Top panel shows the **Room Moducule catalog**: "Standard ICU Room" (3 Moducules: headwall,
   bed, sink).
2. Click **"Stamp into all ICU rooms."**
3. Watch all three ICU rooms flip **EMPTY -> COMPLIANT** at once, Floor 1 roll up to COMPLIANT,
   the building go COMPLIANT, and the impact panel light up — all from ONE reusable template.

> "This is the whole pitch in one click. I designed a Standard ICU Room *once* — a Room
> Moducule — and stamped it into three rooms. Each one ran through the same compliance gate,
> each bed earned its med-gas edge, and all three are now COMPLIANT, rolled up to the floor and
> building. That's 'reusable lego blocks, combined and rearranged to design entire facilities
> while ensuring everything follows the right rules.' Your words. Running."

**The discipline (for the Architect):** the stamp doesn't bypass anything — every component
goes through the identical propose->validate->gate->commit path. If a template component would
violate a rule in a particular room, the gate rejects *that component* and reports it; the rest
still place. Reuse never means skipping validation.

> "And note what it is NOT: it's not a copy-paste of geometry. Each stamp re-validates against
> that room's actual state. Reuse the design, re-earn the compliance. That's the difference
> between a CAD macro and a contextual platform."
