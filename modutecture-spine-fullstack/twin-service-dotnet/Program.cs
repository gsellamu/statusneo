...
builder.Services.AddDbContext<TwinDbContext>(o =>
{
    if (inMemory) o.UseInMemoryDatabase("spine");
        ...
        ?? "Host=localhost;Port=5431;Database=modutecture;Username=postgres;Password=postgres");
});
...
var agentMode = cfg["Agent:Mode"] ?? "ollama";          // deterministic | ollama | reflective
...
{
    Console.WriteLine("[startup] Ollama unreachable - using deterministic brain fallback.");
